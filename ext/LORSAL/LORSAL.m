function [w,L,H1,H2] = LORSAL(x,y, lambda,beta,MMiter,w0)
%       
%
...
if (nargin < 6)
    alpha = 1e-5;
    w=(Rx+alpha*eye(d))\(x*(2*at_val*Y'-at_val*ones(size(Y'))));
% else
%     w=w0;
end
...
%     fprintf('\n i = %d',i);
    ...
    %it's actually not the log-likelihood
    pa = [p;1- sum(p,1)];
    Ya  = [Y;1-sum(Y,1)];
    Ra = ones(size(Ya,1))-eye(size(Ya,1));
    h1 = sum((Ra*pa).*Ya,1);
    h2 = h1;h2(h2>r) = r;
    H2(i) = sum(h2);
    H1(i) = sum(h1);
